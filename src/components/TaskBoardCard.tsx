...
const TaskBoardCard: React.FC<TaskBoardCardProps> = ({ task, milestoneTitle, epicTitle, onToggle, onOpen, draggable, onDragStart, className }) => {
  return (
    <div
      ...
      className={`flex items-start gap-3 py-3.5 border-b border-gray-200 dark:border-white/[0.05] last:border-b-0 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors rounded-lg -mx-2 px-2 group cursor-pointer ${className || ''}`}
    >
      <button
        onClick={(e) => { e.stopPropagation(); onToggle?.(task.id); }}
        className={`
          mt-0.5 flex-shrink-0 w-4 h-4 flex items-center justify-center transition-all rounded-[4px]
          ${task.completed ? 'bg-gray-900 dark:bg-white' : onToggle ? 'border-2 border-gray-400 dark:border-white/25 hover:border-gray-500 dark:hover:border-white/50' : 'border-2 border-gray-300 dark:border-white/10'}
        `}
        disabled={!onToggle}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center flex-wrap gap-x-2 gap-y-1 mt-1.5">
          ...
          <span className="text-[11px] text-gray-400 dark:text-white/30 truncate max-w-[160px]">{epicTitle}</span>
          {task.assignee && (
            <>
              <span className="text-gray-300 dark:text-white/10">·</span>
              <span className="text-[11px] text-gray-400 dark:text-white/30">{task.assignee}</span>
            </>
          )}
          {task.channel && (
            <>
              <span className="text-gray-300 dark:text-white/10">·</span>
              <span className="text-[11px] text-gray-400 dark:text-white/25">{task.channel}</span>
            </>
          )}
          {task.tags.map(tag => {
