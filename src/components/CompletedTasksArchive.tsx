...
const CompletedTasksArchive: React.FC<CompletedTasksArchiveProps> = ({ tasks, onToggle }) => {
  if (tasks.length === 0) {
    return (
      <div className="bg-white dark:bg-[#0f0f11] border border-gray-200 dark:border-white/[0.08] rounded-2xl py-16 text-center">
        <p className="text-[15px] text-gray-500 dark:text-white/30">No completed tasks yet.</p>
        <p className="text-[11px] text-gray-400 dark:text-white/15 mt-1">Finished tasks will appear here.</p>
      </div>
    );
  }
  ...
  return (
    <section className="space-y-5">
      <div className="flex items-baseline justify-between mb-2">
        ...
        <span className="text-[11px] text-gray-400 dark:text-white/25 tabular-nums bg-black/[0.04] dark:bg-white/[0.04] px-2 py-0.5 rounded-md">
          {tasks.length} total
        </span>
      </div>

      {Object.values(byMilestone).map(({ milestone, byEpic }) => (
        <div key={milestone.id} className="bg-white dark:bg-[#0f0f11] border border-gray-200 dark:border-white/[0.08] rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-white/[0.06] flex items-center justify-between">
            <h3 className="text-[14px] font-medium text-gray-700 dark:text-white/70">{milestone.title}</h3>
            ...
            {Object.values(byEpic).map(({ epic, tasks: epicTasks }) => (
              <div key={epic.id}>
                <div className="flex items-center gap-2 mb-2">
                  <h4 className="text-[12px] font-medium text-gray-500 dark:text-white/40">{epic.title}</h4>
                  <span className="text-[10px] text-gray-400 dark:text-white/20 tabular-nums">
                    {epicTasks.length} done
                  </span>
                </div>
                <div className="space-y-1 divide-y divide-gray-200 dark:divide-white/[0.03]">
                  {epicTasks.map(({ task }) => (
                    <div key={task.id} className="flex items-start gap-3 py-2.5 group">
                      <button
                        onClick={() => onToggle(task.id)}
                        className="mt-0.5 flex-shrink-0 w-4 h-4 flex items-center justify-center transition-all rounded-[4px] bg-gray-900 dark:bg-white hover:scale-110 cursor-pointer"
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {task.assignee !== 'Unassigned' && (
                            <>
                              <span className="text-gray-300 dark:text-white/[0.06]">·</span>
                              <span className="text-[10px] text-gray-400 dark:text-white/20">{task.assignee}</span>
                            </>
                          )}
                          <span className="text-gray-300 dark:text-white/[0.06]">·</span>
                          <span className="text-[10px] text-gray-400 dark:text-white/20">{task.channel || 'Task'}</span>
                        </div>
                      </div>
                    </div>
                  ))}
