...
const statusConfig: Record<string, { label: string; cls: string; bg: string; border: string }> = {
  not_started: { label: 'Not Started', cls: 'text-white/35', bg: 'bg-white/[0.03]', border: 'border-white/[0.06]' },
  in_progress: { label: 'In Progress', cls: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
  completed: { label: 'Completed', cls: 'text-green-400', bg: 'bg-green-500/10', border: 'border-green-500/20' },
};
...
const EpicCard: React.FC<EpicCardProps> = ({
  ...
  return (
    <div className="bg-[#131315] border border-white/[0.06] rounded-xl overflow-hidden">
      <div
      ...
      >
        <div className="flex items-center gap-4">
          ...
          {!hasOnlyOneTask && (
            <div className="flex-shrink-0 w-[52px] h-[52px] rounded-xl bg-white/[0.03] border border-white/[0.06] flex flex-col items-center justify-center">
              <div className="text-[17px] font-semibold text-white tabular-nums leading-none">
                {pct}
              </div>
              <span className="text-white/25 text-[9px] mt-0.5">%</span>
            </div>
          )}

          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2.5 mb-1">
              <h4 className="text-[14px] font-medium text-white/85 truncate">{epic.title}</h4>
              <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${status.cls} ${status.bg} ${status.border}`}>
                {status.label}
              </span>
            </div>
            <p className="text-[12px] text-white/30 line-clamp-1 leading-relaxed">{epic.description}</p>
            <div className="flex items-center gap-3 mt-2">
              ...
              <div className="flex items-center gap-1.5 text-[11px] text-white/30">
                <CircleDot size={11} className="text-white/20" />
                <span>{progress.completed}/{progress.total} {progress.total === 1 ? 'task' : 'tasks'}</span>
              </div>
            </div>
          </div>

          <div className="w-8 h-8 rounded-lg bg-white/[0.03] border border-white/[0.06] flex items-center justify-center text-white/30 flex-shrink-0">
      ...
      {isExpanded && (
        <div className="px-5 pb-5 pt-1 border-t border-white/[0.05]">
          {visibleTasks.length === 0 && (
            <p className="text-[12px] text-white/20 py-4 text-center">No open tasks.</p>
          )}
